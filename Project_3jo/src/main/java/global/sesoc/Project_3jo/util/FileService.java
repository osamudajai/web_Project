...
public class FileService {
	...
	public static String saveFile(MultipartFile mfile, String uploadPath) {
		...
		//원본 파일명
		String rv_originalfile = mfile.getOriginalFilename();
		
		//저장할 파일명을 오늘 날짜의 년월일로 생성
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
		String rv_savedfile = sdf.format(new Date());
		
		//원본 파일의 확장자
		String ext;
		int lastIndex = rv_originalfile.lastIndexOf('.');
		//확장자가 없는 경우
		if (lastIndex == -1) {
			ext = "";
		}
		//확장자가 있는 경우
			...
			ext = "." + rv_originalfile.substring(lastIndex + 1);
		}
		...
		while (true) {
			serverFile = new File(uploadPath + "/" + rv_savedfile + ext);
			//같은 이름의 파일이 없으면 나감.
			if ( !serverFile.isFile()) break;	
			//같은 이름의 파일이 있으면 이름 뒤에 long 타입의 시간정보를 덧붙임.
			rv_savedfile = rv_savedfile + new Date().getTime();	
		}		
